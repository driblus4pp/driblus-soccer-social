...
const TeamSorter = ({ onBack }: TeamSorterProps) => {
  const [players, setPlayers] = useState<Player[]>([
    { id: '1', name: 'João', skill: 8, position: 'Atacante' },
    { id: '2', name: 'Pedro', skill: 7, position: 'Meio-campo' },
    { id: '3', name: 'Lucas', skill: 9, position: 'Defesa' },
    { id: '4', name: 'Carlos', skill: 6, position: 'Goleiro' },
  ]);
  ...
  const addPlayer = () => {
    if (newPlayerName.trim()) {
      const newPlayer: Player = {
        ...
        skill: Math.floor(Math.random() * 5) + 5, // Skill entre 5-10
        position: 'Jogador'
      };
  ...
  const updatePlayerSkill = (id: string, skill: number) => {
    setPlayers(players.map(p => 
      p.id === id ? { ...p, skill: Math.max(1, Math.min(10, skill)) } : p
    ));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold">Sorteador de Times 🎲</h1>
      </div>
              ...
              <div key={player.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button 
                    size="sm" 
                    variant="ghost"
                    onClick={() => updatePlayerSkill(player.id, player.skill - 1)}
                  >
      ...
      {teams.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {teams.map((team, index) => (
            <Card key={index} className={`border-2 ${index === 0 ? 'border-blue-300 bg-blue-50' : 'border-red-300 bg-red-50'}`}>
              <CardHeader>
              ...
              <CardContent>
                <div className="space-y-2">
                  {team.players.map((playerName, playerIndex) => (
                    <div key={playerIndex} className="p-2 bg-white rounded flex items-center justify-between">
                ...
                <div className="mt-4 text-center">
                  <Badge className={index === 0 ? 'bg-blue-500' : 'bg-red-500'}>
                    Skill Total: {team.players.reduce((total, playerName) => 
                      total + (players.find(p => p.name === playerName)?.skill || 5), 0
                    )}
                  </Badge>
                </div>
              </CardContent>
