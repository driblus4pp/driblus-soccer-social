...
const BottomNavigation = ({ userType }: BottomNavigationProps) => {
  ...
  const { unreadCount } = userType === 'manager' ? 
    ...
    switch (userType) {
      case 'client':
        return [
          ...
          { icon: Building, label: 'Quadras', path: '/cliente/quadras' },
          { icon: Calendar, label: 'Agenda', path: '/cliente/agendamentos' },
          { icon: Bell, label: 'Avisos', path: '/cliente/notificacoes' },
          { icon: User, label: 'Perfil', path: '/cliente/perfil' },
        ];
      case 'manager':
        return [
          { icon: Home, label: 'Início', path: '/gestor/dashboard' },
          { icon: Calendar, label: 'Agenda', path: '/gestor/agendamentos' },
          { icon: Bell, label: 'Avisos', path: '/gestor/notificacoes', badge: unreadCount },
          { icon: BarChart3, label: 'Relatórios', path: '/gestor/relatorios' },
          { icon: User, label: 'Perfil', path: '/gestor/perfil' },
        ];
      case 'admin':
        return [
          { icon: Home, label: 'Início', path: '/admin/dashboard' },
          { icon: Building, label: 'Quadras', path: '/admin/quadras' },
          ...
          return (
            <Button
              ...
              className={`flex flex-col items-center gap-1 px-2 py-3 h-auto relative ${
                active ? 'text-[#F35410]' : 'text-gray-600'
              }`}
            >
