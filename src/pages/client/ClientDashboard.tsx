...
const ClientDashboard = () => {
  ...
  return <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="px-4 py-6 space-y-6 bg-[#093758]">
        {/* Seção Recomendados para você */}
        <div>
          <h2 className="text-xl font-bold mb-4 text-slate-50">Recomendados para você</h2>
          <div className="space-y-4">
            {recommendedCourts.map(court => <Card key={court.id} className="overflow-hidden cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate(`/cliente/quadra/${court.id}`)}>
                <div className="relative">
                  ...
                  <div className="absolute bottom-3 left-3 right-3">
                    <div className="bg-white/90 backdrop-blur-sm rounded-lg p-3">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button size="sm" className="bg-[#F35410] hover:bg-[#BA2D0B] text-white" disabled={court.status === 'unavailable'}>
        ...
        {filteredCourts.filter(court => !court.isRecommended).length > 0 && <div>
            ...
            <div className="space-y-3">
              {filteredCourts.filter(court => !court.isRecommended).map(court => <Card key={court.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => navigate(`/cliente/quadra/${court.id}`)}>
