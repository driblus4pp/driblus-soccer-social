...
const ClientCourts = () => {
  ...
  const courtData = courts.map(court => ({
    ...
    modality: court.sports.includes(SportType.FOOTBALL) ? 'futebol' : court.sports.includes(SportType.BASKETBALL) ? 'basquete' : 'volleyball',
    operatingHours: '24h',
  ...
  const filteredCourts = courtData.filter(court => {
    ...
    const matchesFilters = (!filters.city || court.city === filters.city) && (!filters.neighborhood || court.neighborhood.includes(filters.neighborhood)) && (!filters.modality || court.modality === filters.modality) && (!filters.operatingHours || court.operatingHours === filters.operatingHours);
    const matchesSportFilter = selectedSportFilter === 'all' || court.sports.includes(sportFilters.find(f => f.id === selectedSportFilter)?.sport!);
  ...
  const getStatusBadge = (status: string, unavailabilityReason?: string) => {
    if (status === 'available') {
      ...
      return <div className="flex flex-col items-end gap-1">
          ...
          {unavailabilityReason && <div className="flex items-center gap-1">
  ...
  return <div className="min-h-screen bg-[#093758] pb-20">
      ...
      <div className="px-4 py-6 bg-[#093758]">
        <div className="flex items-center gap-4 mb-6">
          <h1 className="text-xl font-semibold text-white">Radar de Quadras</h1>
        </div>
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
          {sportFilters.map(filter => <Button key={filter.id} variant="outline" size="sm" onClick={() => setSelectedSportFilter(filter.id)} className={`flex-shrink-0 rounded-full border transition-all duration-200 ${selectedSportFilter === filter.id ? 'bg-[#F35410] border-[#F35410] text-white hover:bg-[#BA2D0B]' : 'bg-white/10 border-white/20 text-white hover:bg-white/20'}`}>
              {filter.label}
            </Button>)}
        </div>
        ...
        {hasActiveFilters && <div className="flex gap-2 mb-2">
            ...
            {selectedSportFilter !== 'all' && <Badge className="bg-white/20 text-white text-xs">
      ...
      <div className="px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredCourts.map(court => <Card key={court.id} className="cursor-pointer hover:shadow-lg transition-all duration-300 bg-white border-0 overflow-hidden group" onClick={() => navigate(`/cliente/quadra/${court.id}`)}>
              <CardContent className="p-0">
                ...
                <div className="p-4">
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {court.sports.slice(0, 3).map((sport, index) => <Badge key={index} variant="secondary" className="bg-blue-100 text-blue-800 text-xs px-2 py-1">
                        {getSportLabel(sport)}
                      </Badge>)}
                    {court.sports.length > 3 && <Badge variant="secondary" className="bg-gray-100 text-gray-600 text-xs px-2 py-1">
                  ...
                  <div className="flex flex-wrap gap-1 mb-4">
                    {court.amenities.slice(0, 3).map((amenity, index) => <span key={index} className="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded-full">
                        {amenity}
                      </span>)}
                    {court.amenities.length > 3 && <span className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full">
                        +{court.amenities.length - 3}
                      </span>}
                  </div>

                  {/* Mensagem de indisponibilidade */}
                  {court.status === 'unavailable' && court.unavailabilityReason && <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button size="sm" className={`flex items-center gap-1 ${court.status === 'available' ? 'bg-[#F35410] hover:bg-[#BA2D0B] text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`} disabled={court.status !== 'available'} onClick={e => {
                  e.stopPropagation();
        ...
        {filteredCourts.length === 0 && <div className="text-center py-12">
