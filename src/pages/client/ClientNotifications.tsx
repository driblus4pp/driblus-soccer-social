...
const ClientNotifications = () => {
  ...
  return <div className="min-h-screen bg-[#093758] pb-20">
      ...
      <div className="px-4 space-y-3">
        {mockNotifications.length === 0 ? <div className="text-center py-12">
          ...
          </div> : mockNotifications.map(notification => {
        const IconComponent = notification.icon;
        return <Card key={notification.id} className={`cursor-pointer transition-all ${notification.isRead ? 'bg-white/5 border-white/10' : 'bg-white/10 border-white/20 shadow-md'}`}>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        {!notification.isRead && <Badge className="bg-[#F35410] text-white text-xs">
                            Nova
                          </Badge>}
                      </div>
                      <p className={`text-sm mb-2 ${notification.isRead ? 'text-white/50' : 'text-white/70'}`}>
