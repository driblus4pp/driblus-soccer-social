...
-- Política para upload de imagens de quadras
CREATE POLICY "Gestores podem fazer upload de quadras" ON storage.objects FOR INSERT WITH CHECK (
  bucket_id = 'quadras' AND
  (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('gestor', 'admin')))
);

-- Política para upload de fotos de perfil
CREATE POLICY "Usuários podem fazer upload de foto de perfil" ON storage.objects FOR INSERT WITH CHECK (
  bucket_id = 'perfis' AND auth.uid() IS NOT NULL
);
