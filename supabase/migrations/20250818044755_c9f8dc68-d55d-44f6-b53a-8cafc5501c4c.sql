...
CREATE TYPE notificacao_tipo AS ENUM ('novo_agendamento', 'agendamento_aprovado', 'reagendamento_sugerido');
...
-- Criar bucket para imagens das quadras
INSERT INTO storage.buckets (id, name, public) VALUES ('quadras', 'quadras', true);

-- Política para upload de imagens
CREATE POLICY "Gestores podem fazer upload" ON storage.objects FOR INSERT WITH CHECK (
  bucket_id = 'quadras' AND
  public.get_current_user_role() IN ('gestor', 'admin')
);

CREATE POLICY "Imagens são públicas" ON storage.objects FOR SELECT USING (bucket_id = 'quadras');