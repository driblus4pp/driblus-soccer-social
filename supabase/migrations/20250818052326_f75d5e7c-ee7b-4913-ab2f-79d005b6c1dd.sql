-- Clean start - drop existing tables safely
...
ALTER PUBLICATION supabase_realtime DROP TABLE quadras;
...
-- Enhanced quadras table with complete structure
...
CREATE POLICY "Admins podem ver todos os perfis" ON profiles FOR ALL USING (get_current_user_role() = 'admin');

CREATE POLICY "Todos podem ver quadras ativas" ON quadras FOR SELECT USING (ativa = true);
CREATE POLICY "Gestores podem gerenciar suas quadras" ON quadras FOR ALL USING (gestor_id = auth.uid() OR get_current_user_role() = 'admin');

CREATE POLICY "Todos podem ver horários de quadras ativas" ON horarios_disponiveis FOR SELECT USING (EXISTS (SELECT 1 FROM quadras WHERE id = quadra_id AND ativa = true));
CREATE POLICY "Gestores podem gerenciar horários de suas quadras" ON horarios_disponiveis FOR ALL USING (EXISTS (SELECT 1 FROM quadras WHERE id = quadra_id AND gestor_id = auth.uid()) OR get_current_user_role() = 'admin');

CREATE POLICY "Usuários podem ver próprios agendamentos" ON agendamentos FOR SELECT USING (cliente_id = auth.uid() OR EXISTS (SELECT 1 FROM quadras WHERE id = quadra_id AND gestor_id = auth.uid()) OR get_current_user_role() = 'admin');
CREATE POLICY "Clientes podem criar agendamentos" ON agendamentos FOR INSERT WITH CHECK (cliente_id = auth.uid());
CREATE POLICY "Gestores podem atualizar agendamentos de suas quadras" ON agendamentos FOR UPDATE USING (EXISTS (SELECT 1 FROM quadras WHERE id = quadra_id AND gestor_id = auth.uid()) OR get_current_user_role() = 'admin');
...
CREATE POLICY "Clientes podem criar avaliações de seus agendamentos" ON avaliacoes FOR INSERT WITH CHECK (cliente_id = auth.uid() AND EXISTS (SELECT 1 FROM agendamentos WHERE id = agendamento_id AND cliente_id = auth.uid() AND status = 'aprovado' AND data <= CURRENT_DATE - INTERVAL '1 day' AND data >= CURRENT_DATE - INTERVAL '7 days'));
...
-- Storage buckets
INSERT INTO storage.buckets (id, name, public) VALUES ('quadras', 'quadras', true) ON CONFLICT (id) DO NOTHING;
INSERT INTO storage.buckets (id, name, public) VALUES ('perfis', 'perfis', true) ON CONFLICT (id) DO NOTHING;

-- Storage policies
CREATE POLICY "Gestores podem fazer upload de quadras" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'quadras' AND (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('gestor', 'admin'))));
CREATE POLICY "Usuários podem fazer upload de foto de perfil" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'perfis' AND auth.uid() IS NOT NULL);
CREATE POLICY "Imagens de quadras são públicas" ON storage.objects FOR SELECT USING (bucket_id = 'quadras');
CREATE POLICY "Fotos de perfil são públicas" ON storage.objects FOR SELECT USING (bucket_id = 'perfis');
